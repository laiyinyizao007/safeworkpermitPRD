...
- **业务能力层**:
  ...
  - **硬约束引擎（新增）**: 四大核心硬约束（互斥逻辑、时效失效、位置漂移、资质强制），不可绕过的系统级强制规则

- **领域核心层**:
  - 作业票聚合根: 8大作业通用属性抽象、生命周期管理、状态机模式
  - 风险识别域: 标准风险库 + 企业自定义风险库 + JSA智能匹配
  - 人员资质域: 证书管理、培训记录、健康档案、生物识别绑定
  - **作业任务聚合（新增）**: DOB NOW 模式下的"一张总票 + 动态子表"核心聚合
...
**DOB NOW 模式支持（新增）**:
- **一张总票 + 动态子表**: 作业任务（Work Task）作为总票，8种作业票作为子表，用户勾选作业类型动态加载
- **硬约束引擎**: 四大核心硬约束（HR-01 互斥逻辑、HR-02 时效失效、HR-03 位置漂移、HR-04 资质强制），双层执行（应用层 + 数据库层）
- **角色动态叠加（简化版）**: 支持2-3个常见角色组合（申请人+负责人、负责人+监护人、审批人+检测人）
- **数据模型扩展**: work_permit_main 表新增 parent_permit_id、is_main_permit、task_sequence、role_stack 字段；permit_association 表新增 relationship_mode 字段
